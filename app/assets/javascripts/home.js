$(function() {
    ...
    var deleteSong = function (event) {
        ...
        $.ajax({
            ...
            data: song,
            success: function (response) {
                $musicList.remove(template.replace("{{title}}", response.title)
                                          .replace("{{artist}}", response.artist)
                                          .replace("{{year}}", response.year)
                                          .replace("{{genre}}", response.genre)
                                          .replace("{{id}}", response.id));
    ...
    var maxLength = function (event) {
        ...
        var titleIngresado = song["title"];
        if(titleIngresado.length == $(this).attr("maxlength")){
